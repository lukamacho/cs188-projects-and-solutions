...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        nearestGhostPos = {0,0}
        nearestGhostDist = 100000000000
        
        ghostPositions =  [distribution.argMax() for distribution in livingGhostPositionDistributions] 

        for ghostP in ghostPositions:
            curDistance = self.distancer.getDistance(pacmanPosition,ghostP)
            if curDistance < nearestGhostDist:
                nearestGhostDist = curDistance
                nearestGhostPos = ghostP

        answerAction = None
        minDistance =10000000000
        for action in legal:
            newPosition = Actions.getSuccessor(pacmanPosition, action)
            curValue = self.distancer.getDistance(newPosition, nearestGhostPos)
